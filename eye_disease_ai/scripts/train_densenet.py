"""
train_densenet.py — Entrenamiento DenseNet121 Transfer Learning Avanzado
========================================================================
Entrena DenseNet121, una arquitectura excelente para imágenes médicas
debido a su reutilización de características (feature reuse) que ayuda
enormemente cuando hay datos limitados.

Implementa mejoras profesionales de nivel élite:
    - WeightedRandomSampler (Resuelve el desbalance de clases)
    - Label Smoothing (Mejora la generalización)
    - TTA (Test Time Augmentation) en evaluación
    - TensorBoard logging dinámico
    - Mixed Precision (AMP) automático con GradScaler
    - Parada Temprana Inteligente con restauración de mejores pesos
    - Dos Fases de Aprendizaje (Solo Cabeza -> Fine-Tuning Completo)
    - Curvas combinadas y matrices de confusión Seaborn (Blues)
    - JSON métricas estandarizado para integración
...
MODEL_DIR    = PROJECT_ROOT / "models" / "densenet"
RESULT_DIR   = PROJECT_ROOT / "results" / "densenet"
...
def get_optimal_config():
    """Retorna configuración óptima según la GPU disponible."""
...
def get_transforms():
    """Transforms optimizados para DenseNet en imágenes de fondo de ojo."""
    return {
...
def build_model(num_classes, freeze_backbone=True):
    """Construye DenseNet121 con cabeza personalizada."""
...
def train_phase(model, dataloaders, dataset_sizes, optimizer, scheduler,
    ...
    """Bucle de entrenamiento de alta robustez."""
    model.to(DEVICE)
...
def evaluate_test(model, dataloaders, class_names, run_name):
    ...
    print(f"\n{'═' * 65}")
    print(f"  📊 REPORTE EN TEST SET — DenseNet121 (con TTA)")
    print(f"{'═' * 65}")
    print(report)
    ...
    # Guardar reporte
    rpt_path = RESULT_DIR / f"densenet_classification_report_{run_name}.txt"
    with open(rpt_path, "w", encoding="utf-8") as f:
        f.write(f"DenseNet121 Classification Report — {run_name}\n{'=' * 65}\n\n")
        f.write(report)
    ...
    ax.set_title(f"DenseNet121 — Matriz de Confusión (Test Set)\nAccuracy: {accuracy*100:.2f}%",
                 fontsize=14, fontweight="bold")
    plt.xticks(rotation=30, ha="right"); plt.yticks(rotation=0)
    plt.tight_layout()
    cm_path = RESULT_DIR / f"densenet_confusion_matrix_{run_name}.png"
    plt.savefig(cm_path, dpi=200, bbox_inches="tight"); plt.close()
    print(f"  📊 Matriz: {cm_path}")

    # JSON
    metrics = {"model": "DenseNet121", "run_name": run_name, "timestamp": datetime.now().isoformat(),
    ...
    mp = RESULT_DIR / f"densenet_metrics_{run_name}.json"
...
def plot_history(h1, h2, run_name):
    ...
    for key, ax, title in [("loss", axes[0], "Loss"), ("acc", axes[1], "Accuracy")]:
        ...
        ax.plot(epochs, train_data, label="Train", linewidth=2, color="dodgerblue")
    ...
    plt.suptitle("DenseNet121 — Curvas de Entrenamiento", fontsize=15, fontweight="bold")
    plt.tight_layout()
    path = RESULT_DIR / f"densenet_training_curves_{run_name}.png"
    plt.savefig(path, dpi=200, bbox_inches="tight"); plt.close()
    print(f"  📈 Curvas: {path}")


def parse_args():
    p = argparse.ArgumentParser(description="Entrenar DenseNet121 para enfermedades oculares")
    p.add_argument("--epochs-head", type=int, default=20, help="Épocas fase 1 (default: 20)")
...
def main():
    ...
    run_name = f"densenet121_{timestamp}"
    
    tb_writer = SummaryWriter(log_dir=str(LOG_DIR / run_name))

    print("\n" + "█" * 65)
    print("  🔬 EYE DISEASE AI — DenseNet121 Transfer Learning")
    ...
    with open(MODEL_DIR / "densenet_classes.json", "w") as f:
    ...
    print("\n" + "█" * 65)
    print("  ✅ ENTRENAMIENTO DENSENET121 AVANZADO COMPLETADO")
