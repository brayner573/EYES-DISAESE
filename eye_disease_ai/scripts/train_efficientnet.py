"""
train_efficientnet.py — Entrenamiento Alta Precisión (Cámara Frontal)
=====================================================================
Utiliza EfficientNetV2-S optimizado para imágenes de celulares:
    - Data Augmentation simulando mala iluminación, desenfoque y ruido.
    - WeightedRandomSampler para desbalance de clases.
    - Mixed Precision y Cosine Annealing.
    - Entrenamiento en 2 Fases (Solo Cabeza -> Fine-Tuning Completo).
    - Parada Temprana Inteligente con restauración de mejores pesos.
    - TensorBoard logging y almacenamiento dinámico.
    - Evaluación exhaustiva con Test Time Augmentation (TTA).
    - Reportes en texto y JSON de métricas detalladas.
    - Visualizaciones premium de entrenamiento y matrices de confusión (Purples).
...
MODEL_DIR    = PROJECT_ROOT / "models" / "efficientnet"
RESULT_DIR   = PROJECT_ROOT / "results" / "efficientnet"
LOG_DIR      = PROJECT_ROOT / "logs"

for d in [MODEL_DIR, RESULT_DIR, LOG_DIR]:
    d.mkdir(parents=True, exist_ok=True)

IMG_SIZE = 224 # EfficientNetV2-S
DEVICE   = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def get_optimal_config():
    """Detecta GPU y retorna configuración óptima para entrenamiento."""
    ...
    if torch.cuda.is_available():
        ...
        cfg["pin_memory"] = True
        if vram >= 16:
            cfg["batch_size"], cfg["num_workers"] = 64, 6
        elif vram >= 12:
            cfg["batch_size"], cfg["num_workers"] = 48, 4
        elif vram >= 8:
            cfg["batch_size"], cfg["num_workers"] = 32, 4
        else:
...
class AddSensorNoise(object):
    """Agrega ruido gaussiano para simular ISO alto de cámara móvil."""
...
class ClampTensor(object):
    """Ajusta los valores del tensor a un rango definido."""
...
def get_mobile_transforms():
    """Transforms extremos simulando fotos de cámaras frontales."""
    return {
        "train": transforms.Compose([
            transforms.Resize((IMG_SIZE + 32, IMG_SIZE + 32)),
            transforms.RandomCrop(IMG_SIZE),
            transforms.RandomHorizontalFlip(p=0.5),
            # Simulaciones de cámara móvil:
            transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.1),
            transforms.RandomRotation(45),
            transforms.RandomAffine(degrees=0, translate=(0.2, 0.2), scale=(0.7, 1.3)),
            transforms.GaussianBlur(kernel_size=5, sigma=(0.1, 2.0)), # Foto movida/desenfocada
            transforms.RandomGrayscale(p=0.1),
            transforms.ToTensor(),
            # Simular ruido de sensor ISO alto (Clases pickleables para Windows compatibility)
            AddSensorNoise(0.05),
            ClampTensor(0.0, 1.0),
...
def build_model(num_classes, freeze_backbone=True):
    """Construye EfficientNetV2-S con cabeza robusta."""
...
class EarlyStopping:
    """Early stopping inteligente que restaura los mejores pesos."""
...
def train_phase(model, dataloaders, dataset_sizes, optimizer, scheduler,
    ...
    """Bucle de entrenamiento con soporte TensorBoard y AMP."""
    model.to(DEVICE)
...
def tta_predict(model, inputs):
    """Test Time Augmentation: Procesa imagen original y flip horizontal."""
    out1 = model(inputs)
...
def evaluate_test(model, dataloaders, class_names, run_name):
    """Evalúa el modelo en test set usando TTA y métricas detalladas."""
    ...
    print(f"\n{'═' * 65}")
    print(f"  📊 REPORTE EN TEST SET — EfficientNetV2-S (con TTA)")
    print(f"{'═' * 65}")
    print(report)
    ...
    # Guardar reporte
    rpt_path = RESULT_DIR / f"efficientnet_classification_report_{run_name}.txt"
    with open(rpt_path, "w", encoding="utf-8") as f:
        f.write(f"EfficientNetV2-S Classification Report — {run_name}\n{'=' * 65}\n\n")
        f.write(report)
    ...
    ax.set_title(f"EfficientNetV2-S — Matriz de Confusión (Test Set)\nAccuracy: {accuracy*100:.2f}%",
                 fontsize=14, fontweight="bold")
    plt.xticks(rotation=30, ha="right"); plt.yticks(rotation=0)
    plt.tight_layout()
    cm_path = RESULT_DIR / f"efficientnet_confusion_matrix_{run_name}.png"
    plt.savefig(cm_path, dpi=200, bbox_inches="tight"); plt.close()
    print(f"  📊 Matriz: {cm_path}")

    # JSON
    metrics = {"model": "EfficientNetV2-S", "run_name": run_name, "timestamp": datetime.now().isoformat(),
    ...
    mp = RESULT_DIR / f"efficientnet_metrics_{run_name}.json"
...
def plot_history(h1, h2, run_name):
    ...
    for key, ax, title in [("loss", axes[0], "Loss"), ("acc", axes[1], "Accuracy")]:
        ...
        ax.plot(epochs, train_data, label="Train", linewidth=2, color="indigo")
        ax.plot(epochs, val_data, label="Val", linewidth=2, color="mediumorchid")
        ax.axvline(sep, linestyle="--", color="gray", alpha=0.7, label="Fine-tune start")
        ax.set_title(title, fontsize=13, fontweight="bold")
        ax.set_xlabel("Época"); ax.set_ylabel(title)
        ax.legend(); ax.grid(alpha=0.3)
    plt.suptitle("EfficientNetV2-S — Curvas de Entrenamiento", fontsize=15, fontweight="bold")
    plt.tight_layout()
    path = RESULT_DIR / f"efficientnet_training_curves_{run_name}.png"
    plt.savefig(path, dpi=200, bbox_inches="tight"); plt.close()
    print(f"  📈 Curvas: {path}")


def parse_args():
    p = argparse.ArgumentParser(description="Entrenar EfficientNet para enfermedades oculares")
    p.add_argument("--epochs-head", type=int, default=15, help="Épocas fase 1 (default: 15)")
    p.add_argument("--epochs-fine", type=int, default=40, help="Épocas fase 2 (default: 40)")
    p.add_argument("--lr-head", type=float, default=1e-3, help="LR fase 1 (default: 1e-3)")
    p.add_argument("--lr-fine", type=float, default=1e-4, help="LR fase 2 (default: 1e-4)")
    p.add_argument("--patience", type=int, default=12, help="Early stopping (default: 12)")
...
def main():
    ...
    run_name = f"efficientnet_{timestamp}"
    
    tb_writer = SummaryWriter(log_dir=str(LOG_DIR / run_name))

    print("\n" + "█" * 65)
    print("  🔬 EYE DISEASE AI — EfficientNetV2-S (Mobile Optimized)")
    print(f"  ⚡ Semilla: {args.seed} | Sampler | Label Smoothing | TTA | 2-Fases")
    print("█" * 65)
    print(f"\n  Device     : {DEVICE}")
    if torch.cuda.is_available():
    ...
    # Datasets
    data_transforms = get_mobile_transforms()
    ...
    with open(MODEL_DIR / "efficientnet_classes.json", "w") as f:
    ...
    model, h2, _ = train_phase(model, dataloaders, dataset_sizes, opt2, sch2,
                               args.epochs_fine, "Fase 2 — Fine-Tuning", cfg["amp"], 
                               args.patience, tb_writer=tb_writer, epoch_offset=offset)

    tb_writer.close()

    # Guardar modelo estructurado
    ...
    print("\n" + "█" * 65)
    print("  ✅ ENTRENAMIENTO EFFICIENTNET AVANZADO COMPLETADO")
