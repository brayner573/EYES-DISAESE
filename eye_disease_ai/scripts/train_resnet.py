"""
train_resnet.py — Entrenamiento ResNet50 Transfer Learning Avanzado
====================================================================
Implementa mejoras profesionales para datasets médicos desbalanceados:
...
MODEL_DIR    = PROJECT_ROOT / "models" / "resnet"
RESULT_DIR   = PROJECT_ROOT / "results" / "resnet"
...
def get_optimal_config():
    """Retorna configuración óptima según GPU disponible."""
...
def get_transforms():
    """Retorna transforms con augmentations mejoradas."""
    return {
        "train": transforms.Compose([
            ...
            transforms.RandomErasing(p=0.2, scale=(0.02, 0.2)), # Más erasing para evitar memorización
        ]),
...
def build_model(num_classes, freeze_backbone=True):
    """Construye ResNet50 con cabeza personalizada para clasificación."""
...
def train_phase(model, dataloaders, dataset_sizes, optimizer, scheduler,
    ...
    """Bucle de entrenamiento con métricas robustas."""
    model.to(DEVICE)
    
    # Label Smoothing reduce la "sobreconfianza" del modelo, mejorando generalización.
...
def tta_predict(model, inputs):
    """Test Time Augmentation: Promedia la predicción de la imagen original y la volteada."""
    # Inferencia original
    out1 = model(inputs)
    # Inferencia con flip horizontal
...
def evaluate_test(model, dataloaders, class_names, run_name):
    ...
    print(f"\n{'═' * 65}")
    print(f"  📊 REPORTE EN TEST SET — ResNet50 (con TTA)")
    print(f"{'═' * 65}")
    print(report)
    ...
    # Guardar reporte
    rpt_path = RESULT_DIR / f"resnet_classification_report_{run_name}.txt"
    with open(rpt_path, "w", encoding="utf-8") as f:
        f.write(f"ResNet50 Classification Report — {run_name}\n{'=' * 65}\n\n")
        f.write(report)
    ...
    ax.set_title(f"ResNet50 — Matriz de Confusión (Test Set)\nAccuracy: {accuracy*100:.2f}%",
                 fontsize=14, fontweight="bold")
    plt.xticks(rotation=30, ha="right"); plt.yticks(rotation=0)
    plt.tight_layout()
    cm_path = RESULT_DIR / f"resnet_confusion_matrix_{run_name}.png"
    plt.savefig(cm_path, dpi=200, bbox_inches="tight"); plt.close()
    print(f"  📊 Matriz: {cm_path}")

    # JSON
    metrics = {"model": "ResNet50", "run_name": run_name, "timestamp": datetime.now().isoformat(),
    ...
    mp = RESULT_DIR / f"resnet_metrics_{run_name}.json"
...
def plot_history(h1, h2, run_name):
    ...
    for key, ax, title in [("loss", axes[0], "Loss"), ("acc", axes[1], "Accuracy")]:
        ...
        ax.plot(epochs, train_data, label="Train", linewidth=2)
        ax.plot(epochs, val_data, label="Val", linewidth=2)
        ax.axvline(sep, linestyle="--", color="gray", alpha=0.7, label="Fine-tune start")
        ax.set_title(title, fontsize=13, fontweight="bold")
    plt.suptitle("ResNet50 — Curvas de Entrenamiento", fontsize=15, fontweight="bold")
    plt.tight_layout()
    path = RESULT_DIR / f"resnet_training_curves_{run_name}.png"
    plt.savefig(path, dpi=200, bbox_inches="tight"); plt.close()
    print(f"  📈 Curvas: {path}")


def parse_args():
    p = argparse.ArgumentParser(description="Entrenar ResNet50 para enfermedades oculares")
...
def main():
    ...
    run_name = f"resnet50_{timestamp}"
    
    tb_writer = SummaryWriter(log_dir=str(LOG_DIR / run_name))

    print("\n" + "█" * 65)
    print("  🔬 EYE DISEASE AI — ResNet50 Transfer Learning")
    print(f"  ⚡ Semilla: {args.seed} | Sampler | Label Smoothing | TTA")
    print("█" * 65)
    print(f"\n  Device     : {DEVICE}")
    if torch.cuda.is_available():
    ...
    with open(MODEL_DIR / "resnet50_classes.json", "w") as f:
    ...
    print("\n" + "█" * 65)
    print("  ✅ ENTRENAMIENTO RESNET50 AVANZADO COMPLETADO")
