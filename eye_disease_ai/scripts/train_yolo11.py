"""
train_yolo11.py — Entrenamiento YOLO11 Classification Avanzado
================================================================
Entrena un modelo YOLO11m-cls (Medium) optimizado para alta precisión
en detección de enfermedades oculares. YOLO11 es la arquitectura más 
reciente y potente de Ultralytics.

Características:
    - Modelo YOLO11m-cls preentrenado (Mayor profundidad y precisión)
    - Early stopping y auto-ajuste de hiperparámetros
    - Data augmentation agresivo (MixUp, Erasing, AutoAugment)
    - Optimizado para GPUs de alta eficiencia con PyTorch CUDA (AMP)
    - Evaluación exhaustiva en test set con promediado de TTA nativo (augment=True)
    - Almacenamiento de reportes detallados y matrices de confusión Seaborn (Greens)
    - Exportación JSON y persistencia en la carpeta de resultados
...
RESULT_DIR   = PROJECT_ROOT / "results" / "yolo11"
...
def print_system_info(gpu_info: dict):
    ...
    if gpu_info["cuda_available"]:
        ...
        print(f"  Batch recom.: {gpu_info['recommended_batch']} (YOLO11 Medium)")
...
# ─── Entrenamiento ────────────────────────────────────────────────────────────
def train_yolo11(args, gpu_info: dict):
    ...
    run_name  = f"yolo11m_eye_cls_{timestamp}"

    print("\n" + "═" * 65)
    print("  🚀 ENTRENAMIENTO YOLO11m — ALTA PRECISIÓN")
    print("═" * 65)
...
def evaluate_model(model, run_name: str):
    ...
    print("\n" + "═" * 65)
    print("  📊 EVALUACIÓN EN TEST SET (YOLO11m)")
    print("═" * 65)
    ...
    for label_idx, cls_name in enumerate(class_names):
        ...
        for img_path in img_files:
            t0 = time.time()
            # Inferencia con augment_inference de YOLOv11 (TTA nativo)
    ...
    # Guardar reporte
    report_path = RESULT_DIR / f"yolo11_classification_report_{run_name}.txt"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"YOLO11m Classification Report — {run_name}\n{'=' * 65}\n\n")
        f.write(report)
    ...
    ax.set_title(f"YOLO11m — Matriz de Confusión (Test)\nAccuracy: {accuracy*100:.2f}%", fontsize=14, fontweight="bold")
    plt.xticks(rotation=30, ha="right"); plt.yticks(rotation=0)
    plt.tight_layout()
    cm_path = RESULT_DIR / f"yolo11_confusion_matrix_{run_name}.png"
    plt.savefig(cm_path, dpi=200, bbox_inches="tight"); plt.close()
    print(f"  📊 Matriz guardada: {cm_path}")

    # JSON metrics
    metrics = {
        "model": "YOLO11m", "run_name": run_name, "timestamp": datetime.now().isoformat(),
    ...
    metrics_path = RESULT_DIR / f"yolo11_metrics_{run_name}.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Entrenar YOLO11 Classification para enfermedades oculares")
    parser.add_argument("--model", type=str, default="yolo11m-cls.pt", help="Modelo base (default: yolo11m-cls.pt)")
...
def main():
    ...
    print("\n" + "█" * 65)
    print("  🔬 EYE DISEASE AI — YOLO11m Classification Training")
    print(f"  ⚡ Semilla: {args.seed} | Modelo YOLO11 Medium | MixUp | AutoAugment")
    print("█" * 65)
    ...
    model, results, run_name = train_yolo11(args, gpu_info)
    ...
    print("\n" + "█" * 65)
    print("  ✅ ENTRENAMIENTO YOLO11m COMPLETADO")
    print("█" * 65 + "\n")
